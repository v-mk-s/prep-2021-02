...
void upd_credit_limit_and_backup(FILE *p_db_clients, FILE *p_transactions, FILE *p_db_clients_backup,
    ...
    while ((fscanf(p_db_clients, "%d", &data_client->number) != -1) &&
            (fscanf(p_db_clients, "%19s%19s", data_client->name, data_client->surname) != -1) &&
            (fscanf(p_db_clients, "%29s%14s", data_client->address, data_client->phone_number) != -1) &&
